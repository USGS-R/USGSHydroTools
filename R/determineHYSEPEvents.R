...
determineHYSEPEvents <- function(HYSEPReturn, sampleDates,percent=0.8){
  ...
  sampleDates$flowConditionHYSEP_localMin <- ifelse(percent*sampleDates$Discharge_cubic_feet_per_second > sampleDates$LocalMin, "Event", "Baseflow")
  sampleDates$flowConditionHYSEP_Sliding <- ifelse(percent*sampleDates$Discharge_cubic_feet_per_second > sampleDates$Sliding, "Event", "Baseflow")
  sampleDates$flowConditionHYSEP_Fixed <- ifelse(percent*sampleDates$Discharge_cubic_feet_per_second > sampleDates$Fixed, "Event", "Baseflow")
